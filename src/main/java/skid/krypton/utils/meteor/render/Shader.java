package skid.krypton.utils.meteor.render;
...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            Krypton.error("Failed to compile vertex shader (" + vertPath + "): " + vertError);
        ...
        if (fragError != null) {
            Krypton.error("Failed to compile fragment shader (" + fragPath + "): " + fragError);
        ...
        if (programError != null) {
            Krypton.error("Failed to link program: " + programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(Identifier.of("krypton","shaders/" + path)).get().getInputStream(), StandardCharsets.UTF_8);
    ...
    public void set(String name, Color color) {
        GL.uniformFloat4(getLocation(name), (float) color.getRed() / 255, (float) color.getGreen() / 255, (float) color.getBlue() / 255, (float) color.getAlpha() / 255);
    }
